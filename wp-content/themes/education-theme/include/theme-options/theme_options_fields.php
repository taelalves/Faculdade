        ...
        foreach ($options as $value) {
            ...
            switch ($value['type']) {
                ...
                case 'text':
                    ...
                    $value['hint_text'] = isset($value['hint_text']) ? $value['hint_text'] : '';
                ...
                case 'textarea':
                    ...
                    if (isset($cs_theme_options[$value['id']]) ? $cs_theme_options[$value['id']] : '') {
                        $val = $cs_theme_options[$value['id']];
                    }
                ...
                case 'generate_backup':
                    ...
                    $output .= '
						<div class="cs-import-help">
							<h4>' . __('Import Options', 'LMS') . '</h4>
						</div>';

                    $output .= '<div class="external_backup_areas">';

                    $output .= '<p>' . __('Input the URL from another location and hit Import Button to apply settings.', 'LMS') . '</p>';

                    $output .= '<input type="text" id="bkup_import_url" />';
                    $output .= '<input id="cs-backup-url-restore" type="button" value="' . __('Import', 'LMS') . '" />';

                    $output .= '</div>';

                    $output .= '
						<div class="cs-import-help">
							<h4>' . __('Export Options', 'LMS') . '</h4>
						</div>';

                    if (is_array($cs_all_list) && sizeof($cs_all_list) > 0) {

                        $output .= '<p>' . __('Here you can Generate/Download Backups. Also you can use these Backups to Restore settings.', 'LMS') . '</p>';
                        ...
                        if (isset($file_val['name'])) {
                            ...
                            $output .= '<input id="cs-backup-restore" data-file="' . $file_val['name'] . '" type="button" value="' . __('Restore', 'LMS') . '" />';

                            $output .= '<a download="' . $file_val['name'] . '" href="' . esc_url($cs_upload_dir_path . $file_val['name']) . '">' . __('Download', 'LMS') . '</a>';

                            $output .= '<input id="cs-backup-delte" data-file="' . $file_val['name'] . '" type="button" value="' . __('Delete', 'LMS') . '" />';
                        }

                        $output .= '</div>';
                    }

                    $output .= '<input type="button" value="' . __('Generate Backup', 'LMS') . '" onclick="javascript:cs_backup_generate(\'' . esc_js(admin_url('admin-ajax.php')) . '\');" />';
                ...
                case 'widgets_backup':
                    ...
                    if (class_exists('cs_widget_data')) {
                        ...
                        $cs_upload_dir = get_template_directory() . '/include/theme-components/cs-widgets/import/widgets-backup/';

                        $cs_upload_dir_path = get_template_directory_uri() . '/include/theme-components/cs-widgets/import/widgets-backup/';

                        $cs_all_list = $wp_filesystem->dirlist($cs_upload_dir);

                        $output .= '
							<div class="cs-import-help">
								<h4>' . __('Import Widgets', 'LMS') . '</h4>
							</div>';
                        ...
                        if (is_array($cs_all_list) && sizeof($cs_all_list) > 0) {
                            ...
                            if (isset($file_val['name'])) {
                                ...
                                $output .= '<input id="cs-wid-backup-restore" data-path="' . $cs_upload_dir_path . '" data-file="' . $file_val['name'] . '" type="button" value="' . __('Show Widget Settings', 'LMS') . '" />';

                                $output .= '<a download="' . $file_val['name'] . '" href="' . esc_url($cs_upload_dir_path . $file_val['name']) . '">' . __('Download', 'LMS') . '</a>';

                                $output .= '<input id="cs-wid-backup-delte" data-file="' . $file_val['name'] . '" type="button" value="' . __('Delete', 'LMS') . '" />';
                            }
                ...
                case 'select':
                    $help_text = '';
                    if (isset($value['hint_text']) && $value['hint_text'] != '') {
                        $help_text = $value['hint_text'];
                    }
                    if (isset($cs_theme_options) and $cs_theme_options <> '') {
                        if (isset($cs_theme_options[$value['id']]) and $cs_theme_options[$value['id']] <> '') {
                            $select_value = $cs_theme_options[$value['id']];
                    ...
                    $output .='<ul class="form-elements" id="' . $value['id'] . '_select">
                                <li class="to-label"><label>' . esc_attr($value['name']) . '<span>' . $help_text . '</span></label></li>
                                    <li class="to-field">
                                    <div class="input-sec">
                                        <div class="select-style">';
                ...
                case 'gfont_select':
                    ...
                    $output .='<option value="default">' . __('Default Font', 'LMS') . '</option>';
                    $i = 0;
                    foreach ($value['options'] as $key => $option) {
                        $selected = '';
                        if ($select_value != '') {
                    ...
                    $output .='<ul class="form-elements" id="' . $value['id'] . '_select">
								<li class="to-label"><label>' . esc_attr($value['name']) . '<span>' . esc_attr($value['hint_text']) . '</span></label></li>
									...
									<div class="input-sec">
										<div class="select-style">
										<select name="' . $value['id'] . '" id="' . $value['id'] . '">
										<option value="">' . __('Select Attribute', 'LMS') . '</option>';
                ...
                case 'select_sidebar':
                    ...
                    $output .= '<ul class="form-elements"><li class="to-label"><label>' . esc_attr($value['name']) . '<span>' . esc_attr($value['hint_text']) . '</span></label></li>
									...
									<div class="input-sec">
										<div class="select-style">
										<select name="' . $value['id'] . '" id="' . $value['id'] . '">';
                    $output .= '<option>' . __('select sidebar', 'LMS') . '</option>';
                ...
                case "upload":
                    ...
                    $output .= '<ul class="form-elements" id="' . $value['id'] . '_upload">
								  ...
								  <li class="to-field">
									...
									<input name="' . $value['id'] . '"  type="button" class="uploadMedia left" value="' . __('Browse', 'LMS') . '"/></label>
								  </li>
								</ul>';
                    //if($val <> ''){			
                ...
                case "upload font":
                    ...
                    $output .= '<ul class="form-elements" id="' . $value['id'] . '_upload">
								  ...
								  <li class="to-field">
									...
									<label class="browse-icon">
										<input name="' . $value['id'] . '" type="button" class="uploadMedia left" value="' . __('Browse', 'LMS') . '"/>
									</label>
								  </li>
								</ul>';
                ...
                case "upload favicon":
                    ...
                    $output .= '<label class="cs-browse"><input id="log" name="' . $value['id'] . '" type="button" class="uploadfile left" value="' . __('Browse', 'LMS') . '"/><i class="fa fa-upload"></i></label></div></li></ul>';
                ...
                case "sidebar":
                    ...
                    $output .= '<ul class="form-elements">
									...
									<li class="to-field">
										...
										<input type="button" value="' . __('Add Sidebar', 'LMS') . '" onclick="javascript:add_sidebar()" />
										<p>' . __('Please enter the desired title of sidebar', 'LMS') . '</p>
									</li>
								</ul>
								<div class="clear"></div>
								<div class="sidebar-area" style="display:' . $display . '">
									<div class="theme-help">
									  <h4 style="padding-bottom:0px;">' . __('Already Added Sidebars', 'LMS') . '</h4>
									  <div class="clear"></div>
									</div>
									<div class="boxes">
										...
										<thead>
											<tr>
												<th>' . __('Sider Bar Nam', 'LMS') . 'e</th>
												<th class="centr">' . __('Actions', 'LMS') . '</th>
											</tr>
										</thead>
                ...
                case "networks":
                    ...
                    $output.='<ul class="form-elements">
								...
								<li class="to-field">
								  ...
								  <p>' . __('Please enter text for icon', 'LMS') . '</p>
								</li>
								<li class="full">&nbsp;</li>
								<li class="to-label">
								...
								<div class="input-sec">
								  ...
								  <label class="browse-icon"><input id="social_net_icon_path_input" name="social_net_icon_path_input" type="button" class="uploadMedia left" value="' . __('Browse', 'LMS') . '"/></label>
							   </li>
							  ...
							  <li class="to-field">
								...
								<li class="to-field" style="width:100%;">
								  <input type="button" value="' . __('Add', 'LMS') . '" onclick=javascript:cs_add_social_icon("' . admin_url("admin-ajax.php") . '") style="float:right;" />
								</li>
							  </ul>
							  <div class="clear"></div>
							  <div class="social-area" style="display:' . $display . '">
							  <div class="theme-help">
								<h4 style="padding-bottom:0px;">' . __('Already Added Social Icons', 'LMS') . '</h4>
								<div class="clear"></div>
							  </div>
							  <div class="boxes">
							  <table class="to-table" border="0" cellspacing="0">
								  <thead>
									<tr>
									  <th>' . __('Icon Path', 'LMS') . '</th>
									  <th>' . __('Network Name', 'LMS') . '</th>
									  <th>' . __('Url', 'LMS') . '</th>
									  <th class="centr">' . __('Actions', 'LMS') . '</th>
									</tr>
								  </thead>
                                ...
                                $output.='<tr id="' . str_replace(' ', '-', $social_net_tooltip[$i]) . '" style="display:none">
													  ...
													  <li><a onclick="cs_toggle(\'' . str_replace(' ', '-', $social_net_tooltip[$i]) . '\')"><img src="' . get_template_directory_uri() . '/include/assets/images/close-red.png" /></a></li>
														  ...
														  <li class="to-field">
															...
															<label class="browse-icon"><input id="social_net_icon_path' . $i . '" name="social_net_icon_path' . $i . '" type="button" class="uploadMedia left" value="' . __('Browse', 'LMS') . '"/></label>
														  </li>
