...
function dateDif($time1, $time2, $precision = 6) {
    if (!is_int($time1)) {
...
function display_tweets($id, $style = '', $max_tweets = 10, $max_cache_tweets = 10, $time = 60) {
    ...
    if (!empty($tweets)) {
        ...
        foreach ($tweets as $tweet) {
            ...
            #Time difference
            $timeDiff = dateDif($today, $pubDate, 1);
            ...
            $twitter .= "<li><div class='text'>" . $tweet . "</p>";

            if (isset($style)) {
                if (!empty($style)) {
                    //$when = ($style == 'time_since' ? 'About' : 'On');
                    $when = ($style == 'time_since' ? '' : '');
                    $twitter.="<span><i class='fa fa-twitter'></i>".$when."";
                    ...
                    $twitter .= "</span></div></li>"; //end of List
                }
            }
	...
	echo "<div class='flexslider flexslider".$rand_id."'><ul class='slides'>".$twitter."</ul></div>";
	
	cs_enqueue_flexslider_script();
		?>
		<script type="text/javascript">
            jQuery(document).ready(function(){
                jQuery(".flexslider<?php echo $rand_id ?>").flexslider({
                    animation: "fade",
                    slideshow: true,
                    slideshowSpeed: 5000,
