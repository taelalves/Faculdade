				...
				echo '<div class="container">';
					...
					if (isset($cs_xmlObject->sidebar_layout) && $cs_xmlObject->sidebar_layout->cs_page_layout <> '' and $cs_xmlObject->sidebar_layout->cs_page_layout <> "none" and $cs_xmlObject->sidebar_layout->cs_page_layout == 'left') : ?>
						<aside class="page-sidebar">
								 <?php if ( !function_exists('dynamic_sidebar') || !dynamic_sidebar(esc_attr($cs_xmlObject->sidebar_layout->cs_page_sidebar_left)) ) : endif; ?>
						 </aside>
					<?php endif; 
					  wp_reset_query();
	...
	if (post_password_required()) {
		 ...
		 */
		 ...
		 foreach ($cs_xmlObject->column_container as $column_container) {
				$cs_section_bg_image = $cs_section_bg_image_position = $cs_section_bg_color = $cs_section_padding_top = $cs_section_padding_bottom = 						$cs_section_custom_style =$cs_section_css_id = $cs_layout = $cs_sidebar_left = $cs_sidebar_right = $css_bg_image = $cs_section_border_color ='';
					...
					if(isset($cs_section_border_bottom) && $cs_section_border_bottom !=''){
					 ...
					 if(isset($cs_section_background_option) && $cs_section_background_option =='section-custom-background-image'){
						...
						$section_style_elements .= 'background: '.$cs_section_bg_imageg.' '.$cs_section_bg_color.';';
							...
							$mute_flag = 'true';
							if($cs_section_ogv_url <> ''){
								$ogv ='<source type="'.cs_video_type($cs_section_ogv_url).'" src="'.$cs_section_ogv_url.'">';
							}else{
								$ogv ='';
							}	
							if($cs_section_webm_url <> ''){
								$webm ='<source type="'.cs_video_type($cs_section_webm_url).'" src="'.$cs_section_webm_url.'">';
							}else{
								$webm ='';
							}	
							...
							if($url['host'] == $_SERVER["SERVER_NAME"]){
 								...
 								$section_video_element = '<div class="page-section-video" style="width: 100%; height:100%; opacity: 1;">
									<video width="100%" height="100%"  id="player'.$rand_player_id.'"  '.$cs_video_autoplay.' loop="true" preload="auto" volume="false" controls="controls" class="nectar-video-bg" style="visibility: visible;"  '.$mute_control.' >
										<source type="'.cs_video_type($cs_section_video_url).'" src="'.$cs_section_video_url.'" >'
										.$ogv.$webm.'
									</video>
			...
			?>
                <!-- Page Section -->
                <section <?php echo cs_allow_special_char($cs_section_css_id);?> class="page-section <?php echo cs_allow_special_char($cs_section_css_class).' '.cs_allow_special_char($parallax_class); ?>" <?php echo cs_allow_special_char($parallax_data_type);?>  <?php echo $section_style_element;?> >
                <?php 
					...
					?>
                            <!-- Container Start -->
                            <div  class="<?php echo cs_allow_special_char($cs_section_view); ?>">
                                <!-- Row Start -->
                                <div class="row">
                                    <?php
												...
												foreach ($column_container->children() as $column) {
													...
													foreach ($column->children() as $cs_node) {
														...
														if ( $cs_node->getName() == "course" ) {
															$isSectionSidebar	= $column_container['cs_layout'];
															global $isSectionSidebar;
															$page_element_size = '';
															...
															echo '</div>';
														} else if ( $cs_node->getName() == "members" ) {
