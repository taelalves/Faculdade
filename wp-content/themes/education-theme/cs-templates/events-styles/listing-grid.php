...
<?php if ( $image_url ) {
		...
		$image_url	= get_template_directory_uri().'/images/no-image16x9.jpg';
	  }?>
<figure>
...
<section class="ev-text">
   ...
   <ul class="ev-location">
	 <?php if( isset( $cs_event_meta->dynamic_post_location_address ) && $cs_event_meta->dynamic_post_location_address <> '' ){ ?>
            <li>
                <i class="fa fa-map-marker"></i> <span><?php echo balanceTags($cs_event_meta->dynamic_post_location_address, false);?></span>
            </li>
    <?php }?>
   </ul>
</section>